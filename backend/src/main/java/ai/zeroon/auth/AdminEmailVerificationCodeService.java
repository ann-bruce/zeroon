...
@Service
public class AdminEmailVerificationCodeService {

    private static final String SUBJECT_PREFIX = "admin-email:";
    ...
    public AdminEmailVerificationCodeService(
    ...
    public void requestCode(String email, String clientIp, boolean deliver) {
        limit("admin-email-code-request-ip-hour", clientIp, ipHourlyLimit, Duration.ofHours(1));
        limit("admin-email-code-request-hour", email, hourlyLimit, Duration.ofHours(1));
        limit("admin-email-code-request-cooldown", email, 1, cooldown);
        if (!deliver) {
            return;
        }
        String code = generator.generate();
        store.store(subject(email), code, ttl);
        sender.send(email, code);
    }

    public boolean verify(String email, String code, String deviceId, String clientIp) {
        limit("admin-email-code-login-device", deviceId, deviceLoginLimit, loginWindow);
        limit("admin-email-code-login-ip", clientIp, ipLoginLimit, loginWindow);
        VerificationCodeStore.VerificationResult result =
                store.verify(subject(email), code, maxFailedAttempts);
