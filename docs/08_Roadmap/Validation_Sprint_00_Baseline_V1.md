...
| RB-06 | P1; S9 active | Long-term memory control loop is incomplete | ADR 004/V9 define controls; S9-02 creates one source-linked Memory after record commit; S9-03 adds owner-only activation, AI-use mutation, and hard deletion with 400/404 boundaries; mobile control and context consumption remain open | Long-term-memory value claim stays blocked until visible mobile controls and context exclusion are accepted | Source-linked memory can be created, viewed, deleted, disabled, and excluded from AI |
