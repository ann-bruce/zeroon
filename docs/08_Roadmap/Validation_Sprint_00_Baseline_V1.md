...
| RB-03 | Engineering closed in S8-03; channel amended 2026-07-24 | Verification code path was local-only and abuse-prone | Prod uses random one-time codes, Redis atomic state, email/IP/device throttling, five-attempt deletion, and fail-closed SMTP delivery; local-only beans are profile-isolated and legacy SMS is disabled by default | Closed-Beta authentication remains blocked until real email delivery, one-time consumption, spam placement, and outage behavior pass in the intended environment | Controller/service/store tests, real Redis cross-instance test, safe prod-profile startup, full quality gate, and intended-environment SMTP acceptance |
