...
| RB-02 | Closed in S8-02 | Production could start with development JWT and database defaults | Prod environment post-processor rejects unsafe JWT, database, and fixed verification-code values | Public release remains blocked by S8-03 and compliance work | Focused tests, unsafe prod boot failure, and full local/test regression passed |
