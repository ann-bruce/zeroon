...
| RB-06 | P1; S9 active | Long-term memory control loop is incomplete | ADR 004 and V9 fix the control model; S9-02 now creates one deterministic source-linked memory after record commit with ownership, idempotency, repair-on-repeat, safe defaults, and failure isolation; mutations remain open | Long-term-memory value claim stays blocked until owner controls and context exclusion are accepted | Source-linked memory can be created, viewed, deleted, disabled, and excluded from AI |
