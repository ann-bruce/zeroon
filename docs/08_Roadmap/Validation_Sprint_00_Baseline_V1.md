...
| RB-06 | P1; S9 active | Long-term memory control loop is incomplete | ADR 004/V9 define controls; S9-02 creates source-linked Memory; S9-03 adds owner-only lifecycle APIs; S9-04 adds mobile source navigation, activation, deletion, and honest read-only AI status; context consumption and editable AI permission remain open | Long-term-memory value claim stays blocked until S9-05 context inclusion/exclusion is accepted | Source-linked memory can be created, viewed, deleted, disabled, and excluded from AI |
